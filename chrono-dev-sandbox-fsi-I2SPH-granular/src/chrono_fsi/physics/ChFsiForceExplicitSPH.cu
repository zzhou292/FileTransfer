...
//--------------------------------------------------------------------------------------------------------------------------------
 __device__ inline Real3 testGradientOperator(  float G_i[9],
                                                Real3 dist3,
                                                Real4 posRadA,
                                                Real4 posRadB,
                                                Real fA,
    ...
    Real Vol = paramsD.markerMass/paramsD.rho0;
    Real fji = fB - fA;
...
//--------------------------------------------------------------------------------------------------------------------------------
 __device__ inline Real4 testLaplacianOperator( float G_i[9],
                                                float L_i[9],
                                                Real3 dist3,
                                                Real4 posRadA,
                                                Real4 posRadB,
    ...
    // Real3 Li_eij;
    // Li_eij.x = L_i[0]*eij.x + L_i[1]*eij.y + L_i[2]*eij.z;
    // Li_eij.y = L_i[3]*eij.x + L_i[4]*eij.y + L_i[5]*eij.z;
    // Li_eij.z = L_i[6]*eij.x + L_i[7]*eij.y + L_i[8]*eij.z;
    ...
    // Real Part1 = Li_eij.x * gradW.x + Li_eij.y * gradW.y + Li_eij.z * gradW.z;
    ...
    Real Part2 = -fji/d  * Vol;
    Real3 Part3 = mR3(-eij.x, -eij.y, -eij.z) * Vol;

    // Real Lap = 2.0*Part1*Part2;

    // Real Lap = -2.0*fji/d*(eij.x*gradW.x + eij.y*gradW.y + eij.z*gradW.z) * Vol;
    return mR4(2.0*Part1*Part2, Part3.x*(2.0*Part1), Part3.y*(2.0*Part1), Part3.z*(2.0*Part1));
    // return mR4(Lap,0.0,0.0,0.0);
}
...
__global__ void Navier_Stokes(Real4* sortedDerivVelRho,  // output: new velocity
            ...
            for (int z = -1; z <= 1; z++) {
                ...
                for (uint j = startIndex; j < endIndex; j++) {
                    if (j != index) {
                        ...
                        // posGra += testGradientOperator(Gi, dist3, sortedPosRad[index], sortedPosRad[j], 
                        //                                pow(posRadA.x,2), pow(posRadB.x,2));
                        // posLap += testLaplacianOperator(Gi, Li, dist3, sortedPosRad[index], sortedPosRad[j], 
                        //                                 pow(posRadA.x,2), pow(posRadB.x,2));
                        ...
                        /*if (rhoPresMuA.w > 0.5 && rhoPresMuB.w < -0.5) {
                            ...
                            Real W_ij = W3h(d, sortedPosRad[index].w);
                            numeratorXxYyZz += tauXxYyZzB * W_ij;
                            numeratorXyXzYz += tauXyXzYzB * W_ij;
                            denominator += W_ij;
                        }*/
                    }
                }
    ...
    /*if (rhoPresMuA.w > 0.5) {
        Real3 tauXxYyZzA = numeratorXxYyZz/denominator;
        Real3 tauXyXzYzA = numeratorXyXzYz/denominator;

        Real3 posRadA_XY = mR3(sortedPosRad[index].x, sortedPosRad[index].y, 0.0);
        Real3 posCylCenter_XY = mR3(paramsD.bodyIniPosX, paramsD.bodyIniPosY, 0.0);
        Real3 distAC = Distance(posRadA_XY, posCylCenter_XY);
        Real dAC = length(distAC);
        Real3 nor_vec = distAC/dAC;
        Real det_s = abs(dAC - paramsD.bodyRad);
        Real spacing = paramsD.MULT_INITSPACE * paramsD.HSML;
        Real3 Force_on_Cyl = mR3(0.0);
        if(det_s < spacing){
            Real Vol_j = spacing*spacing;
            // Real Pre = 1.0/3.0*(tauXxYyZzA.x + tauXxYyZzA.y + tauXxYyZzA.z);
            // Force_on_Cyl = -abs(Pre) * Vol_j * nor_vec;
            Force_on_Cyl.x = -tauXxYyZzA.x * Vol_j * nor_vec.x;
            Force_on_Cyl.y = -tauXxYyZzA.y * Vol_j * nor_vec.y;
            Force_on_Cyl.z = -tauXxYyZzA.z * Vol_j * nor_vec.z;
            // printf(" %f, %f \n", det_s, denominator);
        }
        derivVelRho.x = Force_on_Cyl.x/paramsD.markerMass;
        derivVelRho.y = Force_on_Cyl.y/paramsD.markerMass;
        derivVelRho.z = Force_on_Cyl.z/paramsD.markerMass;
    }*/
    ...
    // posLap.x = posLap.x + posGra.x*posLap.y + posGra.y*posLap.z + posGra.z*posLap.w;
    // if (posGra.x > 0.9 && sortedPosRad[index].x>-0.71  && sortedPosRad[index].x < -0.69 && sortedPosRad[index].z > 0.38) 
    // if (posGra.x < 0.95 && sortedRhoPreMu[index].w < -0.5) 
    //     printf("Finished in %f %f %f, %f %f\n", sortedPosRad[index].x, sortedPosRad[index].z, posGra.x, posGra.y,posGra.z);
    // if ((posLap.x < 2.1 && posLap.x > 1.9) && sortedRhoPreMu[index].w < -0.5 && posRadA.x < -0.5 ) 
    // if ((posLap.x < 2.1 && posLap.x > 1.9) && sortedRhoPreMu[index].w < -0.5 && posRadA.z > 0.37 && posRadA.y > 0.09 ) 
    //     printf("Finished in %f %f %f %f %f %f %f %f %f %f %f %f %f \n", posRadA.x,posRadA.y,posRadA.z, posLap.x, Li[0],Li[1],Li[2],Li[3],Li[4],Li[5],Li[6],Li[7],Li[8]);
    // printf("Finished in %f %f %f %f %f %f %f %f %f\n", Li[0],Li[1],Li[2],Li[3],Li[4],Li[5],Li[6],Li[7],Li[8]);
    ...
    Real det_r_max = length(0.1*velMasA*paramsD.dT);
