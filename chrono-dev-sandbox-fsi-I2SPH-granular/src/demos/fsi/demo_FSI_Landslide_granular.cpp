...
int main(int argc, char* argv[]) {
    ...
    // Set solver settings
    mphysicalSystem.SetSolverMaxIterations(10);
    mphysicalSystem.SetMaxPenetrationRecoverySpeed(1e2);
    mphysicalSystem.SetSolverForceTolerance(0.1);
...
void CreateSolidPhase(ChSystemNSC& mphysicalSystem,
    ...
    /// Create a container
    auto bin = chrono_types::make_shared<ChBody>(ChMaterialSurface::NSC);
    ...
    // AddWall(bin, size_step_x, pos_step_x);
    // AddWall(bin, size_step_y, pos_step_y);
    ...
    // auto bin = std::shared_ptr<ChBody>(mphysicalSystem.NewBody());
    // bin->SetPos(ChVector<>(0, 0, 0));
    // bin->SetBodyFixed(true);
    // bin->SetIdentifier(-1);
    // bin->GetMaterialSurfaceNSC()->SetFriction(friction);
    // bin->SetCollide(true);
    // bin->GetCollisionModel()->SetEnvelope(collision_envelope);
    // bin->GetCollisionModel()->ClearModel();
    // bin->GetCollisionModel()->BuildModel();
    // mphysicalSystem.AddBody(bin);
    ...
    for (int nb = 1; nb < 4; nb++){
        ...
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                ...
                std::shared_ptr<ChBody> house = chrono_types::make_shared<ChBody>(ChMaterialSurface::NSC);
                ...
                house->SetMaterialSurface(mysurfmaterial);
                house->GetCollisionModel()->ClearModel();
                house->GetCollisionModel()->SetSafeMargin(initSpace0);
                chrono::utils::AddBoxGeometry(house.get(), h_size);
                ...
                // house->SetPos(house_pos);
                // house->SetPos_dt(house_vel);
                // house->SetBodyFixed(true);
                // house->SetIdentifier(-1);
                // house->GetMaterialSurfaceNSC()->SetFriction(friction);
                // house->SetCollide(true);
                // house->GetCollisionModel()->SetEnvelope(collision_envelope);
                // house->GetCollisionModel()->ClearModel();
                // house->GetCollisionModel()->BuildModel();
                // myFsiSystem.AddFsiBody(house);
