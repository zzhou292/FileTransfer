...
double cone_length;// = 20.0e-3;//34.36E-3;
double cone_radius;// = 20.0e-3;//9.21E-3;
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
...
int main(int argc, char* argv[]) {
    ...
    std::string inputJson = GetChronoDataFile("fsi/input_json/demo_FSI_ConeDrop_granular.json");
    ...
    cone_radius = paramsH->bodyRad;
    cone_length = paramsH->bodyLength;
    ...
    std::ofstream myFile;
    myFile.open("./Cone_penetration_depth.txt", std::ios::trunc);
    myFile.close();
    myFile.open("./Cone_penetration_depth.txt", std::ios::app);
    myFile << 0.0 << "\t" << 0.0 << "\t"
           << cone->GetPos().x() << "\t" << cone->GetPos().y() << "\t"<< cone->GetPos().z() << "\t"
           << cone->GetPos_dt().x() << "\t" << cone->GetPos_dt().y() << "\t"<< cone->GetPos_dt().z() << "\n";
    myFile.close();
    ...
    for (int tStep = 0; tStep < stepEnd + 1; tStep++) {
        ...
        printf("cone=%f,%f,%f\n", cone->GetPos().x(), cone->GetPos().y(), cone->GetPos().z());
        printf("cone=%f,%f,%f\n", cone->GetPos_dt().x(), cone->GetPos_dt().y(), cone->GetPos_dt().z());
        Real d_pen = fzDim + 0.5*cone_length  + 0.5*initSpace0 - cone->GetPos().z();
        // Real fy = d_pen/pow((0.1 + d_pen), 1.0/3.0);
        printf("cone penetration = %f\n", d_pen);
        myFile.open("./Cone_penetration_depth.txt", std::ios::app);
        myFile << time << "\t" << d_pen << "\t"
               << cone->GetPos().x() << "\t" << cone->GetPos().y() << "\t"<< cone->GetPos().z() << "\t"
               << cone->GetPos_dt().x() << "\t" << cone->GetPos_dt().y() << "\t"<< cone->GetPos_dt().z() << "\n";
...
void CreateSolidPhase(ChSystemSMC& mphysicalSystem,
    ...
    /// Create falling cone
    ChVector<> cone_pos = ChVector<>(paramsH->bodyIniPosX, paramsH->bodyIniPosY, fzDim + 0.5 * cone_length  + 0.5 * initSpace0);
    ChVector<> cone_vel = ChVector<>(0.0, 0.0, -sqrt(abs(2.0*paramsH->gravity.z * paramsH->bodyIniPosZ)));
    ChQuaternion<> cone_rot = QUNIT;
    auto cone = chrono_types::make_shared<ChBody>(ChMaterialSurface::SMC);
    cone->SetPos(cone_pos);
    cone->SetPos_dt(cone_vel);
    double volume = utils::CalcConeVolume(cone_radius, cone_length);
    ChVector<> gyration = utils::CalcConeGyration(cone_radius, cone_length).diagonal();
    double density = paramsH->bodyDensity;//paramsH->rho0 * 0.5;
    double mass = paramsH->bodyMass;//135.7e-3;//141.1e-3 + 0.0e-3;  // density * volume;
    ...
    fsi::utils::AddConeBce(myFsiSystem.GetDataManager(), paramsH, cone, ChVector<>(0, 0, 0), ChQuaternion<>(1, 0, 0, 0),
                           cone_radius, cone_length, paramsH->HSML, false);
