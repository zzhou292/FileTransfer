...
void CreateSolidPhase(ChSystemNSC& mphysicalSystem,
    ...
    /// Create wheels
    ChVector<> Body_Rel_pos_lf = ChVector<>(-1.5, -1.1, 0.4);
    ChVector<> Body_Rel_pos_rf = ChVector<>(-1.5,  1.2, 0.4);
    ChVector<> Body_Rel_pos_lb = ChVector<>( 1.5, -1.1, 0.4);
    ChVector<> Body_Rel_pos_rb = ChVector<>( 1.5,  1.2, 0.4);
    ChVector<> Body_Rel_pos_lf_m = ChVector<>(-0.5, -1.1, 0.4);
    ChVector<> Body_Rel_pos_rf_m = ChVector<>(-0.5,  1.2, 0.4);
    ChVector<> Body_Rel_pos_lb_m = ChVector<>( 0.5, -1.1, 0.4);
    ChVector<> Body_Rel_pos_rb_m = ChVector<>( 0.5,  1.2, 0.4);
    {
    for (int i = 0; i < 8; i++) {
        // load mesh from obj file
        auto mmesh = chrono_types::make_shared<ChTriangleMeshConnected>();
        std::string obj_path = "./tractor_wheel.obj";
        ...
        // Add this body to the FSI system
        myFsiSystem.AddFsiBody(Body);
        std::string BCE_path = "./BCE_tractor_wheel_mid.txt";
        fsi::utils::AddBCE_FromFile(myFsiSystem.GetDataManager(), paramsH, Body, BCE_path, 
                                    ChVector<double>(0), QUNIT, scale_ratio);
    }
    }
...
void SaveParaViewFiles(fsi::ChSystemFsi& myFsiSystem,
    ...
    if (pv_output && std::abs(mTime - (next_frame)*frame_time) < 1e-7) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            char filename[2048];
            if(1==0){// save to obj file
                sprintf(filename, "%s/tractor_wheel_%zd_%zd.obj", paramsH->demo_dir, i+1, next_frame);
                std::vector<geometry::ChTriangleMeshConnected> meshes = { *mmesh };
                geometry::ChTriangleMeshConnected::WriteWavefront(filename, meshes);
            }
            if(1==1){// save to vtk file
                sprintf(filename, "%s/tractor_wheel_%zd_%zd.vtk", paramsH->demo_dir, i+1, next_frame);
                std::ofstream file;
                file.open(filename);
                file << "# vtk DataFile Version 2.0" << std::endl;
                file << "VTK from simulation" << std::endl;
