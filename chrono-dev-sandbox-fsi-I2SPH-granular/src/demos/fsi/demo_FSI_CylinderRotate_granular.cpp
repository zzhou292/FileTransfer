...
Real bxDim = 2.0+ smalldis;
Real byDim = 0.2+ smalldis;
Real bzDim = 0.45+ smalldis;

Real fxDim = 0.7+ smalldis;
Real fyDim = 0.2+ smalldis;
Real fzDim = 0.4+ smalldis;

double cyl_length = 0.1001;
...
void CreateSolidPhase(ChSystemSMC& mphysicalSystem,
    ...
    ChVector<> cyl_pos = ChVector<>(-0.48*bxDim, 0, 1.0*bzDim + 2.0*cyl_radius + 0 * initSpace0);
    ...
    double density = paramsH->rho0 * 0.3;
...
int main(int argc, char* argv[]) {
    ...
    Real initSpace0 = paramsH->MULT_INITSPACE * paramsH->HSML;
    paramsH->cMin = chrono::fsi::mR3(-bxDim / 2 - 5.0*initSpace0/2.0, -byDim / 2 - 1.0*initSpace0/2.0, -1.0*bzDim - 5 * initSpace0) * 1 - 0 * initSpace0;
    paramsH->cMax = chrono::fsi::mR3( bxDim / 2 + 5.0*initSpace0/2.0,  byDim / 2 + 1.0*initSpace0/2.0,  2.0*bzDim + 5 * initSpace0) * 1 + 0 * initSpace0;
