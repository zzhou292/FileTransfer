...
int main(int argc, char* argv[]) {
    ...
    while (step * HEARTBEAT < T_END) {
        ...
        if(rank==1)
        {
            std::cout<<"Sentinel = "<<driver->GetVEHSentinel()<<std::endl;
        }
        ...
        if((step * HEARTBEAT == 2.0) && rank==1)
        {
            driver->ChangeVEHPath(path_2, false);
            driver->SetDesiredSpeed(14.00);
        }
        else if ((step * HEARTBEAT == 8.0) && rank==1)
        {
            driver->ChangeVEHPath(path_1, false);
        }
        else if ((step * HEARTBEAT == 9.0) && rank==1)
        {
