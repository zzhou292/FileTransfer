...
void RCCar_Vehicle::Initialize(const ChCoordsys<>& chassisPos, double chassisFwdVel) {
    ...
    m_axles[0]->Initialize(m_chassis->GetBody(), offset, ChVector<>(0), m_steerings[0]->GetSteeringLink(), 0, 0.0,
                           m_omega[0], m_omega[1]);
    m_axles[1]->Initialize(m_chassis->GetBody(), in2m * ChVector<>(-18.6725, 0, 0), ChVector<>(0), m_chassis->GetBody(),
                           -1, 0.0, m_omega[2], m_omega[3]);
