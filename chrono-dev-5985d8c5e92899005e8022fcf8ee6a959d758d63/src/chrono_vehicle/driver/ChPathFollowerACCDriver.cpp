...
void ChPathFollowerACCDriver::ChangeVEHPath(std::shared_ptr<ChBezierCurve> path, bool isClosedPath)
{
    m_steeringPID.ChangePath(path, isClosedPath);

}

chrono::Vector ChPathFollowerACCDriver::GetVEHSentinel()
{
    return m_steeringPID.GetSentinel();
}
