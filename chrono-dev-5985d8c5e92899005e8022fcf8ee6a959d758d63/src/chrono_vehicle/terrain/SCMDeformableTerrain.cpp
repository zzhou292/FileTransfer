...
void SCMDeformableTerrain::SetMeshSyncing(bool ms) {
    if (GetAutomaticRefinement() && ms) {
        std::cerr << "Mesh syncing cannot be used with auto refinement. Mesh syncing remains off." << std::endl;
    } else {
        m_ground->do_mesh_syncing = ms;
    }
}

bool SCMDeformableTerrain::GetMeshSyncing() const {
    return m_ground->do_mesh_syncing;
}

void SCMDeformableTerrain::SetAutomaticRefinement(bool mr) {
    if (GetMeshSyncing() && mr) {
        std::cerr << "Automatic refinement cannot be used with mesh syncing. Auto refinement remains off." << std::endl;
    } else {
...
// Initialize from path file
void SCMDeformableTerrain::Initialize(std::shared_ptr<ChBezierCurve> path,
                                      const std::string& mesh_name,
                                      double height,
                                      double width,
                                      double dx,
                                      double dy) {
    m_ground->Initialize(path, mesh_name, height, width, dx, dy);
}
...
std::unordered_map<int, chrono::Vector> SCMDeformableTerrain::getMeshDiffs() {
    return m_ground->getMeshDiffs();
}

void SCMDeformableTerrain::applyMeshDiffs(std::unordered_map<int, chrono::Vector> diffs) {
    m_ground->applyMeshDiffs(diffs);
}

void SCMDeformableTerrain::applyMeshDiffs(std::vector<std::pair<int, chrono::Vector>> diffs) {
    m_ground->applyMeshDiffs(diffs);
}
...
void SCMDeformableSoil::Initialize(std::shared_ptr<ChBezierCurve> path,
                                   const std::string& mesh_name,
                                   double height,
                                   double width,
                                   double dx,
    ...
    auto trimesh = m_trimesh_shape->GetMesh();
    unsigned int nv_x = 2 * width / dx + 1;  // Number of vertices in the lateral direction (vertices per segment)
    unsigned int nv_y = 1. / dy;         // Number of vertices in the longitudinal direction (number of segments + 1)
    unsigned int n_verts = nv_x * nv_y;  // Number of total vertices
    unsigned int n_faces = 2 * (nv_x - 1) * (nv_y - 1);  // Number of total faces
    double x_scale = 1.0 / nv_x;
    double y_scale = 1.0 / nv_y;
    ...
    // Load mesh vertices
    // Start at the left boundary and work perpendicularly to the centerline to the right boundary along the entire path
    for (double i = 0; i < 1; i += dy) {
        // Pose at parameter i along the centerline
        // Parameter only derivative calculation
        double par = i;
        ...
        ChVector<> posD = path->evalD((int)td, epar - td);
        ChVector<> pos = path->eval(i);
        for (double j = width; j >= -width; j -= dx) {
            // Calculate vector perpendicular to the current state along the centerline
            ChVector<> dir = Q_from_AngZ(CH_C_PI / 2).Rotate(posD).GetNormalized() * j;

            // Set vertex location
            vertices[it] = plane * (pos + dir);
            vertices[it].z() = height;
            // Initialize vertex noraml to (0,0,0) because assuming flat
            normals[it] = ChVector<>(0, 0, 0);
            // Assign color white to all vertices
            trimesh->getCoordsColors()[it] = ChVector<float>(1, 1, 1);
            // Set UV coordinates in [0,1] x [0,1]
            trimesh->getCoordsUV()[it] = ChVector<>(j * x_scale, i * y_scale, 0.0);
            ;
            it++;
        }
    }
    ...
    // Define the faces
    it = 0;
    for (int i = 0; i < nv_y - 1; i++) {
        for (int j = 0; j < nv_x - 1; j++) {
            // idx_vertices and idx_normals are vectors of ChVectors where each ChVector defines the three points of the
            // face triangle
            int v0 = nv_x * i + j;
    ...
    for (unsigned int i = 0; i < n_faces; ++i) {
        ...
        ChVector<> nrm = Vcross(vertices[idx_vertices[i][1]] - vertices[idx_vertices[i][0]],
                                vertices[idx_vertices[i][2]] - vertices[idx_vertices[i][0]]);
        nrm.Normalize();
        // Increment the normals of all incident vertices by the face normal
        normals[idx_normals[i][0]] += nrm;
        normals[idx_normals[i][1]] += nrm;
        normals[idx_normals[i][2]] += nrm;
        // Increment the count of all incident vertices by 1
        accumulators[idx_normals[i][0]] += 1;
        accumulators[idx_normals[i][1]] += 1;
        accumulators[idx_normals[i][2]] += 1;
    }

    // Set the normals to the average values.
    for (unsigned int i = 0; i < n_verts; ++i) {
        normals[i] = ChWorldFrame::FromISO(normals[i] / (double)accumulators[i]);
    }

    // std::vector<geometry::ChTriangleMeshConnected> v;
    // v.push_back(*m_trimesh_shape->GetMesh());
    // geometry::ChTriangleMeshConnected::WriteWavefront("/Users/aaronyoung/test.obj", v);
    // exit(-1);
...
std::unordered_map<int, chrono::Vector> SCMDeformableSoil::getMeshDiffs() {
    std::unordered_map<int, chrono::Vector> diffs;
    if (!do_mesh_syncing) {
        std::cerr << "getMeshDiffs() called but mesh syncing was disabled; no diffs returned" << std::endl;
        return diffs;
    }

    std::vector<ChVector<>>& vertices = m_trimesh_shape->GetMesh()->getCoordsVertices();

    for (auto& pair : m_changed_vertices) {
        int i = pair.first;
        diffs[i] = vertices[i] - pair.second;
    }

    // Reset the mesh diffs now that we no longer need them
    m_count = 0;
...
void SCMDeformableSoil::applyMeshDiffs(std::unordered_map<int, chrono::Vector> diffs) {
    std::vector<ChVector<>>& vertices = m_trimesh_shape->GetMesh()->getCoordsVertices();

    int m = vertices.size();
    for (auto& pair : diffs) {
        int i = pair.first;
        // NOTE : Keeping the m-i for now since SynChrono doesn't use this version it uses the vector version
        // which has the correct indexing. Useful for testing on one rank
        vertices[m - i] = vertices[m - i] + pair.second;
    }
}

void SCMDeformableSoil::applyMeshDiffs(std::vector<std::pair<int, chrono::Vector>> diffs) {
    std::vector<ChVector<>>& vertices = m_trimesh_shape->GetMesh()->getCoordsVertices();

    for (auto& pair : diffs) {
        int i = pair.first;
        vertices[i] = vertices[i] + pair.second;
    }
}
...
void SCMDeformableSoil::ComputeInternalForces() {
    ...
    for (auto& p : patches) {
        ...
        // Code to collect info about Convex Hull failures. For debugging; don't keep longterm
        auto& hpoints = ch.GetHull();
        std::cout.precision(18);
        std::cout << std::scientific;
        if (hpoints.size() > p.points.size() + 1) {
            std::cout << "Reached Convex Hull Failure" << std::endl;
            std::cout << std::endl;
            for (auto p_in : p.points)
                std::cout << p_in.x() << "  " << p_in.y() << std::endl;
            std::cout << std::endl;
            for (auto h_out : hpoints)
                std::cout << h_out.x() << "  " << h_out.y() << std::endl;
            std::cout << std::endl;
            // exit(1);    // if possible (e.g. not in an MPI code), bail out immediately
        }
    ...
    for (auto& h : hits) {
        ...
        if (p_sigma[i] < 0) {
            ...
            // At this point we're already in an if-statement that guarantees the node changed
            if (do_mesh_syncing) {
                m_count++;
                // unordered_map::insert does NOT overwrite if the key is already present whereas the bracket operator
                // does. We don't want to overwrite since we need the state of vertices[i] as our record of the mesh
                // before it changed (p_vertices_initial gets reset every step, m_changed_vertices does not)

                m_changed_vertices.insert({i, vertices[i]});
            }
    ...
    if (do_refinement) {
        ...
        for (int it = 0; it < idx_vertices.size(); ++it) {
            ...
            if (p_sigma[idx_vertices[it][0]] > 0 || p_sigma[idx_vertices[it][1]] > 0 ||
                ...
                m_marked_tris.push_back(it);
            }
        }

        m_num_marked_faces = m_marked_tris.size();
        ...
        for (int i = 0; i < 1; ++i) {
            m_trimesh_shape->GetMesh()->RefineMeshEdges(
                m_marked_tris, refinement_resolution, &refinement_criterion,
                ...
                for (const auto& ivc : connected_vertexes[is]) {
                    ...
                    if (true) {
                        if (p_massremainder[is] > p_massremainder[ivc]) {
                            ...
                            if (do_mesh_syncing) {
                                // See note above, we don't want to overwrite existing content in m_changed_vertices
                                m_changed_vertices.insert({ivc, vertices[ivc]});
                                m_changed_vertices.insert({is, vertices[is]});
                            }
