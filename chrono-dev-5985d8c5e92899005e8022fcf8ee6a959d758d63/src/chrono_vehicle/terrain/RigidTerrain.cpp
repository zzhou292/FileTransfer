...
void RigidTerrain::LoadPatch(const rapidjson::Value& d) {
    ...
    if (d["Geometry"].HasMember("Dimensions")) {
        ...
        patch =
            AddPatch(material, ChCoordsys<>(loc, rot), vehicle::GetDataFile(bmp_file), mesh_name, sx, sy, hMin, hMax);
    }
...
std::shared_ptr<RigidTerrain::Patch> RigidTerrain::AddPatch(std::shared_ptr<ChMaterialSurface> material,
                                                            ...
                                                            double height,
                                                            int width,
                                                            double dx,  // Lateral interval
                                                            double dy,  // Longitudinal interval
                                                            double sweep_sphere_radius,
                                                            bool visualization) {
    auto patch = chrono_types::make_shared<MeshPatch>();
    AddPatch(patch, position, material);
    ...
    for (double i = 0; i < 1; i += dy) {
        ...
        for (double j = width; j >= -width; j -= dx) {
            ...
            patch->m_trimesh->getCoordsColors()[it] = ChVector<float>(1, 1, 1);
            // Set UV coordinates in [0,1] x [0,1]
            patch->m_trimesh->getCoordsUV()[it] = ChVector<>(j * x_scale, i * y_scale, 0.0);
            ;
            it++;
        }
    }
...
std::shared_ptr<RigidTerrain::Patch> RigidTerrain::AddPatch(std::shared_ptr<ChMaterialSurface> material,
    ...
    std::vector<geometry::ChTriangleMeshConnected> meshes = {*patch->m_trimesh};
    geometry::ChTriangleMeshConnected::WriteWavefront("/Users/aaronyoung/test.obj", meshes);
