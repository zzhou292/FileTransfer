...
class CH_VEHICLE_API RigidTerrain : public ChTerrain {
  public:
    ...
    /// Add a terrain patch generated from path file.
    /// The mesh is specified through a path file and is used for both contact and visualization.
    std::shared_ptr<Patch> AddPatch(
        std::shared_ptr<ChMaterialSurface> material,  ///< [in] contact material
        const ChCoordsys<>& position,                 ///< [in] patch location and orientation
        std::shared_ptr<ChBezierCurve> path,          ///< [in] path to generate a patch from
        const std::string& mesh_name,                 ///< [in] name of the mesh asset
        double height,                                ///< [in] height of the terrain
        int width,                                    ///< [in] width from the specified path to an outer boundary
        double dx,                                    ///< [in] Lateral interval
        double dy,                                    ///< [in] Longitudinal interval
        double sweep_sphere_radius = 0,               ///< [in] radius of sweep sphere
        bool visualization = true                     ///< [in] enable/disable construction of visualization assets
    );
