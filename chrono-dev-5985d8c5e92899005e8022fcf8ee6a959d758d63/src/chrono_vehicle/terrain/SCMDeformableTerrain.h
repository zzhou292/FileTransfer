...
class CH_VEHICLE_API SCMDeformableTerrain : public ChTerrain {
  public:
    ...
    /// Enable/disable the saving of mesh data for the purpose of synchronizing it with a different ChSystem
    /// Currently incompatible with automatic refinement, checked and enforced when either setter is called
    void SetMeshSyncing(bool ms);
    bool GetMeshSyncing() const;
    ...
    /// The initial undeformed mesh is generated from the provided path file
    void Initialize(std::shared_ptr<ChBezierCurve> path,  ///< [in] path to generate a patch from
                    const std::string& mesh_name,         ///< [in] name of the mesh asset
                    double height,                        ///< [in] terrain height
                    double width,                         ///< [in] width of the generated mesh
                    double dx,  ///< [in] number of divisions in the direction perpendicular to the path
                    double dy   ///< [in] number of divisions in the direction parallel to the path
    );
    ...
    std::unordered_map<int, chrono::Vector> getMeshDiffs();

    void applyMeshDiffs(std::unordered_map<int, chrono::Vector> diffs);
    void applyMeshDiffs(std::vector<std::pair<int, chrono::Vector>> diffs);
...
class CH_VEHICLE_API SCMDeformableSoil : public ChLoadContainer {
  public:
    ...
    /// Map of the index and z-deformation for all nodes in the mesh that have deformed (had their z-coordinate changed)
    /// since the last time this function was called. Used to synchronize changes between two separate copies of a mesh,
    /// e.g. call this function on your own mesh and send the result to the other mesh you wish to synchronize. See also
    /// applyMeshDiffs
    std::unordered_map<int, chrono::Vector> getMeshDiffs();

    /// Applies z-deformations [double] to nodes based on their index [int]. Used to synchronize changes between two
    /// separate copies of a mesh, e.g. call this function with diffs that you received from a mesh you want to
    /// synchronize with. See also getMeshDiffs
    void applyMeshDiffs(std::unordered_map<int, chrono::Vector> diffs);
  ...
  private:
    ...
    bool do_mesh_syncing;

    std::unordered_map<int, chrono::Vector> m_changed_vertices;
