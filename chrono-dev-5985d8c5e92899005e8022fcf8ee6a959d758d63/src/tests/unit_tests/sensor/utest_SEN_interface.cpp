...
TEST(ChSensorManager, box_intersect) {
    ...
    auto cam = chrono_types::make_shared<ChCameraSensor>(
        boxA, 50, chrono::ChFrame<double>({-8, 0, 3}, Q_from_AngAxis(0, {1, 0, 0})), 1, 1, 1, 0, .01);
...
TEST(ChSensorManager, sphere_intersect) {
    ...
    auto c = chrono_types::make_shared<ChSensorManager>(&mphysicalSystem);
    auto cam = chrono_types::make_shared<ChCameraSensor>(
        sphere, 50, chrono::ChFrame<double>({-8, 0, 3}, Q_from_AngAxis(0, {1, 0, 0})), 1, 1, 1, 0, .01);
    cam->SetName("Camera Sensor");
    c->AddSensor(cam);
...
TEST(ChSensorManager, mesh_intersect) {
    ...
    auto c = chrono_types::make_shared<ChSensorManager>(&mphysicalSystem);
    auto cam = chrono_types::make_shared<ChCameraSensor>(
        dummy, 50, chrono::ChFrame<double>({-8, 0, 3}, Q_from_AngAxis(0, {1, 0, 0})), 1, 1, 1, 0, .01);
    cam->SetName("Camera Sensor");
    c->AddSensor(cam);
...
TEST(OptixGeometry, cylinder) {
    ...
    c->scene->AddPointLight({5, -5, 10}, {1, 1, 1}, 100);
    auto cam = chrono_types::make_shared<ChCameraSensor>(
        cylinder, 50, chrono::ChFrame<double>({-8, 0, 2.5}, Q_from_AngAxis(.4, {0, 1, 0})), 1280, 720, CH_C_PI / 3, 0,
        .01);
    cam->SetName("Camera Sensor");
