...
int main(int argc, char* argv[]) {
    ...
    reflective_color->SetSpecularColor({1.0, 1.0, 1.0});
    ...
    manager->scene->AddPointLight({0, 0, 100}, {1, 1, 1}, 500);
    ...
    auto cam = std::make_shared<ChCameraSensor>(
        ...
        chrono::ChFrame<double>({-10, 0, 0}, Q_from_AngAxis(0, {0, 0, 1})),  // offset pose
        1920,                                                                // image width
        1080,                                                                // image height
        CH_C_PI / 1.2, 0.02, 0);                                             // FOV
