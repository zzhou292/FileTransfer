...
void InferenceDriver::Initialize() {
    ...
    std::shared_ptr<float> in1 = std::shared_ptr<float>(
        cudaMallocHelper<float>(m_input_img->Height * m_input_img->Width * 3 * sizeof(float)), cudaFreeHelper<float>);
    m_process_buffers[0] = in1.get();

    std::shared_ptr<float> in2 =
        std::shared_ptr<float>(cudaMallocHelper<float>(m_input_2->size() * sizeof(float)), cudaFreeHelper<float>);
