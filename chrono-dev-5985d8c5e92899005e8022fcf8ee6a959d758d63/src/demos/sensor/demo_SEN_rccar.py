...
# ax[0].imshow(np.zeros((100,100,3)))
# ax[0].set_title('Ground Truth')
# ax[1].imshow(np.zeros((100,100,3)))
# ax[1].set_title('Prediction')
# ax[2].imshow(np.zeros((100,100,3)))
...
def ControlCar(steering,velocity,img,lidar_depth,pt_cloud):
    ...
    # pt_cloud = pt_cloud[pt_cloud[:,:,2]>z_ground_param]
    ...
    # print("max depth at x id=",x," target steering:",target_s)
    # img[0,0,0]
    ...
    # cv2.imshow("front camera",np.zeros((100,100,4)))
    # cv2.waitKey(0)
    # f = plt.figure()
...
camera_3rd = sens.ChCameraSensor(
    ...
    chrono.ChFrameD(chrono.ChVectorD(-2.5, 0, .9), chrono.Q_from_AngAxis(.2, chrono.ChVectorD(0, 1, 0))),  # offset pose
    ...
    0.01,   #lag
    0   #collection window/shutter speed
)
...
camera_front = sens.ChCameraSensor(
    my_rccar.GetChassisBody(),                                                          # body lidar is attached to
    60,                                                                 # scanning rate in Hz
    chrono.ChFrameD(chrono.ChVectorD(0, 0, .2), chrono.Q_from_AngAxis(0, chrono.ChVectorD(0, 1, 0))),  # offset pose
    210*2,                                                               # number of horizontal samples
    160*2,                                                                 # number of vertical channels
    chrono.CH_C_PI / 4,                                                            # horizontal field of view
    (160/210)*chrono.CH_C_PI / 4.,                                                        # vertical field of view
    0.01,0
)
