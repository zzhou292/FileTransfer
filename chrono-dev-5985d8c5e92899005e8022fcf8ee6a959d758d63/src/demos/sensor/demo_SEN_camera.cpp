...
int main(int argc, char* argv[]) {
    ...
    // ------------------------------------------------
    chrono::ChFrame<double> offset_pose1({-5, 0, 2}, Q_from_AngAxis(.2, {0, 1, 0}));
    ...
    auto cam2 = chrono_types::make_shared<ChCameraSensor>(mesh_body,      // body camera is attached to
                                                          update_rate,    // update rate in Hz
    ...
    while (ch_time < end_time) {
        ...
        if (rgba8_ptr->Buffer) {
            std::cout << "RGBA8 buffer recieved from cam. Camera resolution: " << rgba8_ptr->Width << "x"
                      << rgba8_ptr->Height << ", frame= " << rgba8_ptr->LaunchedCount << ", t=" << rgba8_ptr->TimeStamp
                      << std::endl
                      << std::endl;
        }
