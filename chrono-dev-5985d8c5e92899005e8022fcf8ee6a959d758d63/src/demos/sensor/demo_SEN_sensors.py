...
def main():
    ...
    cam = sens.ChCameraSensor(
        ...
        cam_lag,                # lag
        cam_collection_time     # collection window
    )
    ...
    lidar = sens.ChLidarSensor(
        ...
        lidar_lag,              # lag
        lidar_collection_time,  # collection time
    )
    ...
    imu = sens.ChIMUSensor(box,                     # body imu is attached to
                           ...
                           offset_pose,             # offset pose
                           imu_lag,                 # lag
                           imu_collection_time,     # collection window
    ...
    gps = sens.ChGPSSensor(box,                     # body imu is attached to
                           ...
                           offset_pose,             # offset pose
                           gps_lag,                 # lag
                           gps_collection_time,     # collection window
                           gps_reference
    ...
    while (ch_time < end_time):
        ...
        camera_data_RGBA8 = camera.GetMostRecentRGBA8Buffer()
        camera_data_R8 = camera.GetMostRecentR8Buffer()
...
vertical_fov = chrono.CH_C_PI / 6.    # 30 degrees
