...
__global__ void image_alias_kernel(unsigned char* bufIn,
    ...
    // float w_pixels_to_merge = w_in / (float)w_out;
    // float h_pixels_to_merge = h_in / (float)h_out;
    ...
    if (out_index < w_out * h_out * pix_size) {
        ...
        // int idc_in = idc_out;
        // int idx_in = idx_out * factor;
        // int idy_in = idy_out * factor;
        //
        // int in_index = idy_in * w_in * pix_size + idx_in * pix_size + idc_out;
        //
        // bufOut[out_index] = bufIn[in_index];
