...
RT_PROGRAM void pbr_shader() {
    ...
    if (has_normal_map) {
        float3 normal_delta = make_float3(tex2D(normal_map, texcoord.x, texcoord.y));  // change to normal
        forward_normal = normalize(forward_normal + (normal_delta * 2 - make_float3(1)));
        //        forward_normal = normal_delta;
        //        printf("%f %f %f\n", normal_delta.x, normal_delta.y, normal_delta.z);
    ...
    // ==========================================================
    float3 resulting_color = tmp_kd * ambient_light_color;
    ...
    if (roughness < .99) {
        ...
        if (reflect_importance > importance_cutoff && prd_camera.depth < max_depth) {
            ...
            resulting_color += reflect_importance * prd_reflection.color;
        }
    }
    ...
    if (transparency < .99) {
        ...
        if (refract_importance > importance_cutoff && prd_camera.depth < max_depth) {
            ...
            rtTrace(root_node, refraction_ray, prd_refraction, RT_RAY_FLAG_DISABLE_ANYHIT);
            resulting_color += refract_importance * prd_refraction.color;
        }
    }
        ...
        if (dist_to_light < 2 * l.max_range) {
            ...
            if (fmaxf(light_attenuation) > 0.0f) {
                ...
                if (NdL_result > 1e-6f) {
                    ...
                    resulting_color += (1 - refract_importance) * ((make_float3(1) - Ks) * f_lambert + Ks * f_ct) *
                                       l.color * light_attenuation * NdL_result;
                }
            }
        }
    }

    prd_camera.color = resulting_color;
}
