...
ChOptixEngine::ChOptixEngine(ChSystem* sys, int device_id, int max_scene_reflections, bool verbose)
    : m_verbose(verbose), m_deviceId(device_id), m_recursions(max_scene_reflections) {
    m_system = sys;
    Initialize();
}
...
void ChOptixEngine::AssignSensor(std::shared_ptr<ChOptixSensor> sensor) {
    ...
    {
        // lock mutex to wait for engine to finish job before adding a new sensor
        // std::unique_lock<std::mutex> tmp_lock = std::unique_lock<std::mutex>(m_renderQueueMutex);
...
void ChOptixEngine::UpdateSensors(std::shared_ptr<ChScene> scene) {
    ...
    for (int i = 0; i < m_assignedSensor.size(); i++) {
        ...
        if (m_system->GetChTime() > sensor->GetNumLaunches() / sensor->GetUpdateRate() - 1e-7) {
            // time to start packing the keyframes
            // TODO: be careful because the worker thread may still be using the keyframes
...
void ChOptixEngine::staticTrimeshVisualization(std::shared_ptr<ChTriangleMeshShape> trimesh_shape,
    ...
    unsigned int* tmp_index_buffer = (unsigned int*)(index_buffer->map());
...
void ChOptixEngine::UpdateCameraTransforms() {
    for (auto sensor : m_assignedSensor) {
        std::shared_ptr<ChOptixSensor> pOptixSensor = std::dynamic_pointer_cast<ChOptixSensor>(sensor);
        if (pOptixSensor) {
            Program ray_gen = pOptixSensor->m_ray_gen;
