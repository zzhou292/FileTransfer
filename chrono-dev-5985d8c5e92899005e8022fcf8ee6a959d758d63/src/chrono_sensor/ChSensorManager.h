...
class CH_SENSOR_API ChSensorManager {
  public:
    ...
    /// @param verbose  Whether the sensor manager should be explicit about its operations
    /// @param render_ray_reflections How many reflections (bounces) are allowed in the OptiX scene if one should be
    /// created.
    ChSensorManager(ChSystem* chrono_system, bool verbose = false, int render_ray_reflections = 7);
